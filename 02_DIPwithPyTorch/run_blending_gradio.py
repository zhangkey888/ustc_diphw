...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    laplacian_kernel = torch.tensor([
        ...
        [1, -4, 1],
        [0, 1, 0]
    ], dtype=torch.float32, device=foreground_img.device).view(1, 1, 3, 3)

    # 对每个通道分别进行卷积操作
    foreground_laplacian = torch.zeros_like(foreground_img)
    blended_laplacian = torch.zeros_like(blended_img)

    for i in range(foreground_img.size(1)):  # 遍历通道
        foreground_laplacian[:, i, :, :] = F.conv2d(foreground_img[:, i:i + 1, :, :], laplacian_kernel, padding=1)
        blended_laplacian[:, i, :, :] = F.conv2d(blended_img[:, i:i + 1, :, :], laplacian_kernel, padding=1)

    # foreground_laplacian = foreground_laplacian[:, :, :blended_laplacian.size(2), :blended_laplacian.size(3)]
    # blended_laplacian = blended_laplacian[:, :, :foreground_laplacian.size(2), :foreground_laplacian.size(3)]
    # 计算损失，只考虑掩码区域内的像素
    loss = (foreground_laplacian - blended_laplacian) ** 2
    loss = loss * foreground_mask * background_mask  # 只考虑掩码区域内的像素
    loss = loss.sum() / (foreground_mask * background_mask).sum()  # 归一化损失
