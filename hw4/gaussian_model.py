...
class GaussianModel(nn.Module):
    ...
    def compute_covariance(self) -> torch.Tensor:
        ...
        scales = torch.exp(self.scales)  # Convert from log scale to regular scale
        S = torch.diag_embed(scales)    # Create a diagonal matrix for the scaling factors
    
        # Compute covariance
        # Covariance matrix for each Gaussian is the form:
        # Cov = R * S * R^T, where R is the rotation matrix and S is the diagonal scaling matrix.
        # This ensures that the covariance matrix incorporates both rotation and scale.
         ### FILL:
        Covs3d = torch.matmul(R, S)  # Rotate the scaling matrix
        Covs3d = torch.matmul(Covs3d, R.transpose(1, 2))  # Apply the inverse rotation
